...
module.exports = () => {
  ...
  ipcMain.on("fetch-categories", (evt, authUUID, blogName) => {
    ...
    api.fetchCategories(auth.authInfo, blogName).then((categories) => {
  ...
  ipcMain.on("fetch-posts", (evt, authUUID, blogName, options) => {
    ...
    api.fetchPosts(auth.authInfo, blogName, options).then((posts) => {
  ...
  ipcMain.on("save-content", (evt, authUUID, blogName, post) => {
    ...
    api.savePost(auth.authInfo, blogName, post).then((res) => {
  ...
  ipcMain.on("add-content", (evt, authUUID, blogName, post) => {
    ...
    api.addPost(auth.authInfo, blogName, post).then((res) => {

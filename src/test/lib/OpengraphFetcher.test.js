...
it('Opengraph Fetch', async done => {
	...
	await OpengraphFetcher.fetch("http://notice.tistory.com/2384", opengraph => {
...
it('Youtube', async done => {
	...
	await OpengraphFetcher.fetch("https://youtu.be/8szi8_383JI", opengraph => {
		expect(opengraph.title).toBe(expected.title)
		expect(opengraph.description).toBe(expected.description)
		expect(opengraph.image).toBe(expected.image)
		expect(opengraph.type).toBe(expected.type)
...
it('None opengraph', async done => {
	await OpengraphFetcher.fetch('https://www.tinymce.com/docs/api/tinymce/tinymce.shortcuts/', opengraph => {
